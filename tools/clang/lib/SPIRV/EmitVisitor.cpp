...
bool EmitVisitor::visit(SpirvExecutionMode *inst) {
  ...
  if (inst->getopcode() == spv::Op::OpExecutionMode) {
    ...
    for (SpirvInstruction *param : inst->getIdParams()) {
      if (auto *constArg = dyn_cast<SpirvConstant>(param)) {
        curInst.push_back(typeHandler.getOrCreateConstant(constArg));
      } else {
...
uint32_t EmitTypeHandler::emitType(const SpirvType *type) {
  ...
  // NodePayloadArray types
    ...
    if (const auto *attr = nodeDecl->getAttr<HLSLMaxRecordsAttr>()) {
      auto instrs = context.getEvaluatedAttrArgs(attr);
      if (instrs.size() > 0 && instrs[0] &&
          instrs[0]->getopcode() == spv::Op::OpSpecConstant) {
        maxRecords = getOrAssignResultId<SpirvInstruction>(instrs[0]);
      } else {
        maxRecords =
            getOrCreateConstantInt(llvm::APInt(32, attr->getMaxCount()),
  ...
  // Structure types
    ...
    if (const auto recordDecl = dyn_cast_or_null<RecordDecl>(
      ...
      for (auto &pair : context.getGraphicsNodeDecorations(recordDecl)) {
        emitDecoration(id, pair.second, {}, pair.first);
      }
    }
